...
class Table(Generic[T]):
    ...
    def get(self, index: tuple[int,int], default: Optional[T2]=None) -> T | T2 | None:
    ...
    def reshape(self, src: 'Table[T2]', dst: 'Table[T2]', default: T3) -> 'Table[T | T3]':
    ...
    @classmethod
    def Shape(cls, shape: TableShape, values: Mapping[tuple[int,int], T] | Iterable[T], default: T) -> 'Table[T]':
    ...
    def Parse(cls,
        ...
        while q:
            ...
            y1 = y0 + 1
            if r_seps:
                while y1<=h and not any((y1,x) in all_seps for x in range(x0+1,x1)):
                    y1 += 1
...
def format_table(table:Table[Any], sep: str='|', pad: str=' ', just: Callable[[str,int], str] = cjust) -> str:
    ...
    for (r0,c0,_,c1), content, _size in _render(table, h_sep_width=len(sep), h_pad_width=len(pad), v_sep_width=0):
        if len(content) > 1:
            raise ValueError(f'cannot format table with multiline cells: {content!r}')
        lines[r0] = _edit_str(lines[r0], c0, sep + just('\n'.join(content), c1-c0-1) + sep)
...
def format_boxed_table(table: Table[Any], pad: str=' ', just: Callable[[str,int],str]=cjust) -> str:
...
def _render(table:Table[T], h_sep_width: int=1, h_pad_width: int=1, v_sep_width: int=1, str_f: Callable[[T], str]=str):
