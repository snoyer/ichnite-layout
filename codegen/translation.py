...
class Translator(Generic[T]):
    ...
    def register_aliases(
        self, aliases: dict[str, str | Callable[[re.Match[str]], str]]
    ):
    ...
    def register_translations(
        self, translations: dict[str, T | Callable[[re.Match[str]], T]]
    ):
    ...
    def translate_all(
        self, keys: Sequence[Key], is_layer_name: Callable[[str], bool]
    ) -> list[T]:
...
def translate_multi_os_layers(
    ...
    return {
        name: translator_by_os[layer_to_os[name]].translate_all(
            keys, is_layer_name=layers.__contains__
        )
...
def shorten_layers_names(
    ...
    return {
        shorten_layer_name(k): [
            update_layer_names(shorten_layer_name, b) for b in bindings
        ]
        for k, bindings in layers.items()
    }
...
def dedup_keymaps_layers(
    ...
    tmp_bindings: dict[tuple[int, str], list[T]] = {
        (i, name): bindings for i, (name, bindings) in enumerate(layers.items())
    }
    ...
    while True:
        ...
        for indexed_names in duplicates:
            ...
            tmp_bindings[(indexed_names[0][0], new_name)] = tmp_bindings[
                indexed_names[0]
            ]
        ...
        if new_ids:
            tmp_bindings = {
                k: [
                    update_layer_names(lambda layer: new_ids.get(layer, layer), b)
                    for b in bindings
                ]
                for k, bindings in tmp_bindings.items()
            }
        else:
            break
