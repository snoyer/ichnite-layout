...
from .source import (
    Key,
    Keymap,
    extract_os_specifics_from_md,
    join_layer_name,
    keymap_from_md,
    make_multi_os_layers,
    split_layer_name,
)
from .translation import (
    Translator,
    dedup_keymaps_layers,
    shorten_layers_names,
    translate_multi_os_layers,
)
...
def main(
    ...
    if default_key is not None:
        ...
        new_layers = {
            name: [b if b else default_key for b in layer] for name, layer in layers
        }
    ...
    if args.command == "ZMK":
        ...
        uc_modes_by_os = {
            shorten_layer_name(
                join_layer_name(base_name, ["mac"])
            ): "UNICODE_MODE_MACOS",
            shorten_layer_name(
                join_layer_name(base_name, ["linux"])
            ): "UNICODE_MODE_WINDOWS",
            shorten_layer_name(
                join_layer_name(base_name, ["win"])
            ): "UNICODE_MODE_LINUX",
        }
        generate_code = partial(
            generate_qmk_code, layout_name=args.layout, uc_modes_by_base=uc_modes_by_os
        )
    else:
    ...
    translators_by_os = {
        os: translators_postproc(translator_for_os(os))
        for os in os_specific_codes.keys()
    }
    first_translator = next(iter(translators_by_os.values()))
    translated_layers = translate_multi_os_layers(
        source_multi_layers, translators_by_os.__getitem__, first_os_from_layer_name
    )
    translated_layers = dedup_keymaps_layers(
        translated_layers, first_translator.map_layer_names, ignore=dont_dedup
    )
    translated_layers = shorten_layers_names(
        translated_layers, first_translator.map_layer_names, shorten_layer_name
    )
